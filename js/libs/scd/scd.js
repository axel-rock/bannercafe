 ...
 */
export default (videoEl, options, callback) => {
	 ...
	 * @const
	 */
	const MAX_DIFF_100 = 4.4167 // Math.sqrt(255 * 255 * 3) / 100;
	 ...
	 * @type {boolean}
	 */
	const controls = videoEl.controls
	 ...
	 * @type {number}
	 */
	let canvasContextImageDataLength
	...
	function init() {
		...
		const _step_sq = opts.step_width * opts.step_height
	...
	function detectSceneChange() {
		...
		const diff = computeDifferences(ctxA, ctxB)

		if (diff > opts.threshold) {
			// Trigger a `scenechange` event.
			const _sceneChangeEvent = document.createEvent('Event')
			_sceneChangeEvent.initEvent('scenechange', true, true)
			videoEl.dispatchEvent(_sceneChangeEvent)

			const /** @type {Element} */ tmpContainer = document.createElement('div'),
				/** @type {HTMLCanvasElement} */ tmpCanvasA = createCanvas()
			tmpCanvasA.width = width
			tmpCanvasA.height = height
			tmpCanvasA.getContext('2d').drawImage(videoEl, 0, 0, videoEl.videoWidth, videoEl.videoHeight, 0, 0, width, height)

			sceneTimecodes.push({ timecode: currentTime, diff: Math.round(diff / MAX_DIFF_100), canvas: tmpCanvasA })
			if (DEBUG && opts.debug) {
	...
	function computeDifferences(ctxA, ctxB) {
		...
		for (; i < canvasContextImageDataLength; i = i + 4) {
			diff.push(
				getColorDistance(colorsA[i], colorsA[i + 1], colorsA[i + 2], colorsB[i], colorsB[i + 1], colorsB[i + 2])
			)
		}
	...
	function formatTime(num) {
		...
		let seconds = (num - hours * 3600 - minutes * 60).toFixed(2)
